"""catalog application logic. Talks to its own store and the event stream only."""
...
def handle(action, **kwargs):
    line("catalog", kwargs.get("correlation_id", "-"), f"catalog.{action}")
    if action == "list":
        return []
    if action == "get":
        return None
...
def emit(bus, event, payload, correlation_id):
    return publish(bus, f"catalog.{event}", payload, correlation_id)
