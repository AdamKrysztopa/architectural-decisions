"""notifications application logic. Talks to its own store and the event stream only."""
...
def handle(action, **kwargs):
    line("notifications", kwargs.get("correlation_id", "-"), f"notifications.{action}")
    if action == "list":
        return []
    if action == "get":
        return None
...
def emit(bus, event, payload, correlation_id):
    return publish(bus, f"notifications.{event}", payload, correlation_id)
