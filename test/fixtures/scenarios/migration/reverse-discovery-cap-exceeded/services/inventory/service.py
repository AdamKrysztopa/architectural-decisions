"""inventory application logic. Talks to its own store and the event stream only."""
...
def handle(action, **kwargs):
    line("inventory", kwargs.get("correlation_id", "-"), f"inventory.{action}")
    if action == "list":
        return []
    if action == "get":
        return None
...
def emit(bus, event, payload, correlation_id):
    return publish(bus, f"inventory.{event}", payload, correlation_id)
